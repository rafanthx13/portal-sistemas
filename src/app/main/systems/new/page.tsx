...
export default function NewSystemPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.message || 'Erro ao criar sistema');
      }

      setSuccess(true);
      setTimeout(() => {
        router.push('/main/systems');
      }, 2000);
    } catch (err) {
      console.error('Erro ao criar sistema:', err);
      setError(err instanceof Error ? err.message : 'Ocorreu um erro ao criar o sistema. Tente novamente.');
    } finally {
      setIsSaving(false);
    }
  };
      ...
      <main className="min-h-screen bg-gray-100">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="bg-white shadow rounded-lg p-6">
            <h2 className="text-2xl font-semibold text-gray-800 mb-6">Novo Sistema</h2>
            ...
            {success && (
              <div className="mb-4 p-4 bg-green-50 border border-green-200 text-green-600 rounded-lg">
                Sistema criado com sucesso! Redirecionando...
              </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex justify-end space-x-4">
                ...
                <button
                  ...
                  disabled={isSaving}
                >
                  {isSaving ? 'Salvando...' : 'Criar Sistema'}
                </button>
              </div>
            </form>
          </div>
